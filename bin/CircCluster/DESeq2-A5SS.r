...
#step2: normalize
RC<-read.table(file="A5SS.expression",header=F,row.names=1)
names(RC)<-RMS
...
condition=c(rep("case",3),rep("contaol",3))
...
write.table(resultRC,file="A5SS.DESeq.xls",row.names=TRUE,sep="\t",quote=FALSE)
